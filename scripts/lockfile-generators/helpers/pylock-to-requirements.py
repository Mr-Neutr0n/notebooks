...
Arguments:
    ...
    --sdist-hashes    el9-fallback (default): omit sdist hashes when an EL9-compatible
                      ...
                      prefer: include sdist hashes unless EL9-compatible wheels cover
                      every Konflux arch present in the lockfile's wheels. Public-index
                      baselines still fall back to sdists when a locked arch has no
                      manylinux wheel, but omit Rust sdists (uv, rpds-py) that fail
                      Hermeto ``cargo vendor --locked``.
...
def collect_index_hashes(pkg: dict, *, sdist_hashes: str = SDIST_HASHES_EL9_FALLBACK) -> list[str]:
    ...
    ``prefer``: add sdist hashes unless EL9 wheels cover every Konflux arch the
    package marker installs on (so ppc64le/s390x can still install when an
    ungated arch has no manylinux wheel, without fetching uv/rpds-py sdists
    that fail Hermeto cargo vendoring).
    """
    hashes: list[str] = []
    has_el9_wheel = False
    ...
    if sdist_hashes == SDIST_HASHES_PREFER:
        required_arches = konflux_arches_required(str(pkg.get("marker", "")))
        include_sdist = not required_arches.issubset(covered_arches)
    else:
        include_sdist = not has_el9_wheel
    if include_sdist:
        sdist = pkg.get("sdist")
