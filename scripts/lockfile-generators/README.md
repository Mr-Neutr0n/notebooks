...
| ----------------------------------------------- | ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `helpers/pylock-to-requirements.py`             | pip              | Convert `pylock.<flavor>.toml` (PEP 751) to pip-compatible `requirements.<flavor>.txt` with `--hash` lines. Default `--sdist-hashes el9-fallback` omits sdist hashes when an EL9-compatible wheel exists; public-index uses `--sdist-hashes prefer` (sdist omitted when EL9 wheels cover every Konflux arch the package marker installs on). |
